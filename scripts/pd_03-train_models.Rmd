...
mod_variance <- cbind(model = c("lm", "rf", "glm", "glmb", 
                                "gam", "gbm", "svm"), 
                      mod_variance)
mod_variance
```
...
Mgamuk1a.full <- train(PEAT_DEPTH ~ ., 
              data = training, 
              method = "gam")

  modeltime <- rbind(modeltime, 
                     data.frame(model = "Mgamuk1a.full",
                                insType = #if not on EC2, use local
                                  "local",
                                  #aws.ec2metadata::metadata$instance_type(),
...
save(Mgamuk1a.full, file = "../data/models/Mgamuk1a.full.rds")
...
Mgbmuk1a.full <- train(PEAT_DEPTH ~ ., 
              data = training, 
              method = "gbm")

  modeltime <- rbind(modeltime, 
                     data.frame(model = "Mgbmuk1a.full",
                                insType = #if not on EC2, use local
                                  "local",
                                  #aws.ec2metadata::metadata$instance_type(),
                                time_mins = difftime(Sys.time(), starttime, units = "mins")))
  
save(Mgbmuk1a.full, file = "../data/models/Mgbmuk1a.full.rds")
...
evaluate
```{r}
rbind(getTrainPerf(Mlmuk1a.full), getTrainPerf(Mrfuk1a.full), getTrainPerf(Mgamuk1a.full), getTrainPerf(Mgbmuk1a.full))
```
