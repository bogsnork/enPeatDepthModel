...
# rboristGrid <-  expand.grid(#nTree= 1,
#                             minNode= 30,
#                             predFixed = 2)
#params <- trainControl(method = "cv", number = 1)

  starttime <- Sys.time()

Mrfuk1ah <- train(PEAT_DEPTH ~ .,
              data = training,
              method = "ranger", 
              num.trees = 100)
              

#using Rborist direct (not with caret) to tune parameters for quicker processing
#tried different number of trees, 200 trees (model Mrfuk1ag) gave only marginal improvement over 100 (model Mrfuk1af) so settled on 100 trees.

# Mrfuk1af <- Rborist(x = training[-1], y = training$PEAT_DEPTH, 
#                    nTree=100,
#                    minNode=3,
#                    predFixed = 2)

  modeltime <- rbind(modeltime, 
                     data.frame(model = "Mrfuk1ah",
                                insType = aws.ec2metadata::metadata$instance_type(),
                                time_mins = difftime(Sys.time(), starttime, units = "mins")))  
...
#using Rborist direct (not with caret) to tune parameters for quicker processing
#tried different number of trees, 200 trees (model Mrfuk1ag) gave only marginal improvement over 100 (model Mrfuk1af) so settled on 100 trees. both took under 30 minutes. 
# 
# Mrfuk1af <- Rborist(x = training[-1], y = training$PEAT_DEPTH, 
#                    nTree=100,
#                    minNode=3,
#                    predFixed = 2)
  ...
  modeltime <- rbind(modeltime, 
                     data.frame(model = "Mrfuk1ah",
                                insType = aws.ec2metadata::metadata$instance_type(),
                                time_mins = difftime(Sys.time(), starttime, units = "mins")))  
save(Mrfuk1ah, file = "../data/models/Mrfuk1ah.rds")

Mrfuk1ah
# Mrfuk1af$training$call
# Mrfuk1af$validation[2:4]
# Mrfuk1ag$training$call
# Mrfuk1ag$validation[2:4]
# 
# Mrfuk1a <- Mrfuk1af
